...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userId = res.data.userId || res.data.id || res.data._id || res.data.user?._id;
      if (!token || !userId) {
        throw new Error('Missing token or user ID in response:', JSON.stringify(res.data));
      }
      ...
      toast.success('Logged in successfully!');
      // Delay redirect to ensure token is set
      setTimeout(() => navigate('/dashboard'), 100); // Small delay to ensure state update
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.message || 'Login failed';
      setError(errorMessage);
      toast.error(errorMessage);
      console.error('Login error:', errorMessage, 'Response:', err.response?.data, 'Status:', err.response?.status);
    } finally {
      setIsSubmitting(false);
    }
  };
